...
function getUniqueResources(resources) {
  const stringifiedResources = resources
    .map(resource => {
      return JSON.stringify(resource);
    })
  const uniqueResources = unique(stringifiedResources);
  return uniqueResources
    .map(stringifiedResource => {
...
export function getLoaderConfig(renderingInfos, env) {
  let loaderConfig = {};
  Object.keys(renderingInfos)
    .forEach(itemId => {
      const renderingInfo = renderingInfos[itemId];
      if (renderingInfo.loaderConfig === undefined) {
        return;
      }
      if (renderingInfo.loaderConfig.loadSystemJs === 'production' || renderingInfo.loaderConfig.loadSystemJs === 'full') {
      ...
      if (Array.isArray(renderingInfo.loaderConfig.polyfills)) {
        ...
        loaderConfig.polyfills = loaderConfig.polyfills.concat(renderingInfo.loaderConfig.polyfills);
      }
    })
...
export function getStylesheetsToLoad(renderingInfos, env) {
  ...
  Object.keys(renderingInfos)
    .forEach(itemId => {
      const renderingInfo = renderingInfos[itemId];
      if (renderingInfo.stylesheets === undefined || !Array.isArray(renderingInfo.stylesheets)) {
        return;
      }
      const stylesheetsForItem = renderingInfo.stylesheets
        .map(stylesheet => resolvePath(stylesheet, env));
...
export function getScriptsToLoadOnce(renderingInfos, env) {
  ...
  Object.keys(renderingInfos)
    .forEach(itemId => {
      const renderingInfo = renderingInfos[itemId];
      if (renderingInfo.scripts === undefined || !Array.isArray(renderingInfo.scripts)) {
        return;
      }
