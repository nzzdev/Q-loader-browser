...
function loadGraphics(qItemElements) {
  ...
  Promise.all(renderingInfoPromises)
    ...
    .then(renderingInfos => {
      scriptsToLoadOnce = scriptsToLoadOnce.concat(
        getScriptsToLoadOnce(renderingInfos, env)
      );
      return renderingInfos;
    })
    .then(renderingInfos => {
      scriptsToLoadPerItem = getScriptsToLoadPerItem(renderingInfos, env);
      return renderingInfos;
    })
    .then(renderingInfos => {
      loadScripts(scriptsToLoadOnce).then(() => {
