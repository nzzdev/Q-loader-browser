...
describe('helpers.getStylesheetsToLoad', () => {
  it('should merge and unique the stylesheet configs', function() {
    return helpersImported
      .then(helpers => {
        let stylesheetsToLoad = helpers.getStylesheetsToLoad(renderingInfosMock, env);
        expect(stylesheetsToLoad.length).to.be.equal(2);
        expect(stylesheetsToLoad[0].url).to.be.equal('https://service.sophie.nzz.ch/bundle/sophie-q@^1,sophie-color@^1.css');
        expect(stylesheetsToLoad[1].url).to.be.equal('http://q-server.st.nzz.ch/foo/bar.css');
      })
  })
})
...
describe('helpers.getScriptsToLoadOnce', () => {
  it('should merge and unique the script configs', function() {
    return helpersImported
      .then(helpers => {
        let scriptsToLoadOnce = helpers.getScriptsToLoadOnce(renderingInfosMock, env);
...
describe('helpers.getScriptsToLoadPerItem', () => {
  it('should remove scripts from item scripts that should load only once per page', function() {
    return helpersImported
      .then(helpers => {
        let scriptsToLoadPerItem = helpers.getScriptsToLoadPerItem(renderingInfosMock, env);
  ...
  it('should keep equal scripts if they should get loaded/executed per item', function() {
    return helpersImported
      .then(helpers => {
        let scriptsToLoadPerItem = helpers.getScriptsToLoadPerItem(renderingInfosMock, env);
        expect(Object.keys(scriptsToLoadPerItem).length).to.be.equal(2);
        expect(scriptsToLoadPerItem['1'][0].content).to.be.equal('load for every item script');
        expect(scriptsToLoadPerItem['3'][0].content).to.be.equal('load for every item script');
      })
  })
})
